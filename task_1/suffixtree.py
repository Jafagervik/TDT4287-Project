# suffix tree code & node retrieved from:
# https://github.com/kasravnd/SuffixTree/blob/master/SuffixTree/suffixtree.py
...
class Node:
    ...
    # following get functions added by fbrate.
...
class SuffixTree:
    ...
    # matchS implemented by fbrate for reverse match of S.
    def matchS(self, s):
        if(len(s) == 0):
            return 0
        length = 0
        ...
        edge_children = self.root.getChildren()
        current_edge = None
        for child in edge_children:
            cur_s = s[len(s) - length - 1]
            if child == s[len(s) - length - 1]:
                # matching new edge
                current_edge = edge_children[child]
                length += 1
                #saved_length = 1
                break
        while current_edge is not None:

            # skip first element in length because we already checked that
            edge_start_index = current_edge.getStart()+1
            edge_end_index = current_edge.getEnd()
            i = edge_start_index

            while i <= edge_end_index and length < len(s):
                a_char = r_a[i]
                s_char = s[len(s)-length-1]
                if a_char == s_char:
                    length +=1
                    i += 1
                elif a_char == '$':
                    ...
                    # not a match break loop here.
                    current_edge = None
                    break
            if current_edge is not None:
                # means all in iteration was true.
                # check for $ -> if exists update saved length
                    # else return
                # in the same loop also check for new edge
                edge_children = current_edge.getChildren()
                current_edge = None
                if len(edge_children) != 0:
                    for child in edge_children:
                        cur_s = s[len(s)-length-1]
                        if child == s[len(s)-length-1]:
                            # matching new edge
                            current_edge = edge_children[child]
                            length += 1
                        # if a child contains end tag store it
                        elif child == '$':
                            if current_edge is None:
                                saved_length = length
                            else:
                                saved_length = length - 1
                else:
                    # if no more children and last string is terminating char. Save length
                    if r_a[edge_end_index] == '$':
                        saved_length = length
        return saved_length
